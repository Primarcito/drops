...
def ephemeral_key(interaction: discord.Interaction, scope: str):
    ...
    return (int(interaction.user.id), str(guild_id), str(channel_id), str(scope))
...
async def _acknowledge_hidden(interaction: discord.Interaction):
...
async def _dismiss_current_original(interaction: discord.Interaction):
    with suppress(discord.HTTPException, discord.NotFound):
